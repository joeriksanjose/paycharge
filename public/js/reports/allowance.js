$(document).ready(function(){
	...
	$("#btn-search").click(function(){
		...
		$.post(base_url+"reports/allowance/search_award",{search:search}, function(data){
			...
			if(json_data.length>0){
				$.each(json_data, function(i, item){
					...
					str = str + '<td>';
					str = str + '<a target="_blank" class="btn edit-award-btn" href="'+base_url+'reports/allowance/print_modern/'+item.trans_no+'/'+item.modern_award_no+'">';
	...
	$("#btn-search-client").click(function(){
		...
		$.post(base_url+"reports/allowance/search_client",{search:search}, function(data){
			...
			if(json_data.length>0){
				$.each(json_data, function(i, item){
					...
					str = str + '<td>'+item.created_at+'</td>';
					str = str + '<td>';
					str = str + '<a target="_blank" class="btn edit-award-btn" href="'+base_url+'reports/allowance/print_client/'+item.trans_no+'">';
					str = str + '<i class="icon-print"></i></a>';
					str = str + '</td>';
					str = str + '</tr>';
